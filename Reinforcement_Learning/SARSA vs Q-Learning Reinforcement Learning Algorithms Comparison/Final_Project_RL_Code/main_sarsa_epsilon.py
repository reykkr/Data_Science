...
sarsaQEpsilon9 = { game.s0 : { a:0.0 for a in game.actions }}
sarsaQEpsilon6 = { game.s0 : { a:0.0 for a in game.actions }}
sarsaQEpsilon2 = { game.s0 : { a:0.0 for a in game.actions }}
sarsaEpisodeScore9 = []
sarsaEpisodeScore6 = []
sarsaEpisodeScore2 = []
sarsaAverageScore9 = []
...
for i in range(n):
  ...
  sarsaQEpsilon9, sarsaScore = sarsa.playEpisode(sarsaQEpsilon9, 
                                  game.s0,
                                  game.actions,
  ...
  sarsaQEpsilon6, sarsaScore = sarsa.playEpisode(sarsaQEpsilon6, 
                                  game.s0,
                                  game.actions,
                                  game.observe_next_state, 
                                  game.observe_reward, 
  ...
  sarsaQEpsilon2, sarsaScore = sarsa.playEpisode(sarsaQEpsilon2, 
                                  game.s0,
                                  game.actions,
                                  game.observe_next_state, 
                                  game.observe_reward, 
  ...
  if i%100==0 :
    ...
    plt.clf() # Important...
    plt.plot(sarsaAverageScore9, label="reward sarsa epsilon 0.9")
    plt.plot(sarsaAverageScore6, label="reward sarsa epsilon 0.6")
    plt.plot(sarsaAverageScore2, label="reward sarsa epsilon 0.2")
    plt.legend(loc=4) #bottom right
    plt.title('sarsa epsilon')
    plt.ylabel('Average reward/Episode per 100 episodes')
    plt.xlabel('Episodes')
    plt.savefig( '~outputEpsilonsarsa.png' )
          #plt.show()  



sarsa.printQ(sarsaQEpsilon9)
sarsa.printQ(sarsaQEpsilon6)
sarsa.printQ(sarsaQEpsilon2)
