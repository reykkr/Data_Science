...
def chooseAction(epsilon, state, actions, Q) :
...
def playEpisode(Q, s0, actions, observe_next_state, observe_reward, isEnd, epsilon= 0.9, alpha= 0.1, horizon= 1000, gamma=0.9):
    ...
    while not isEnd(state) and h < horizon :
        action, typeAction = chooseAction(epsilon, state, actions, Q)
        ...
        nextAction, typeNextAction = chooseAction(epsilon, next_state, actions, Q)
        # update rule for Q-learning
        Q[state][action] = ( ( 1-alpha ) * Q[state][action] ) + ( alpha * ( reward + (gamma * Q[next_state][nextAction] ) ) )
